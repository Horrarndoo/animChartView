...
public class HistoryChartView_old extends View {
	...
	private int mMode = 1;// ��Activity��������ģʽֵ 1���� 2���� 3���� 4����
	...
	private void drawRect(Canvas canvas, float[] data, int dataColor) {
		...
		for (int i = 0; i < Xlabel.length; i++) {
			...
			if (i != 0) {
				...
				if (currentBarProgress[i] > top + 10) {
	...
	private void startTargetTempAnimation() {
		...
		targetTempValueAnimator = ValueAnimator.ofFloat(0,
				mTargetTempPathMeasure.getLength());
		LogUtils.info(TAG,
